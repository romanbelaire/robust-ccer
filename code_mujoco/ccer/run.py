...
def main(params):
    ...
    if 'exp_id' in params.keys():
        store = Store(base_directory, exp_id=params['exp_id'])
    else:
        store = Store(base_directory)

    if 'exp_id' not in params.keys():
        print('making repo')
    ...
    if params['mode'] == "adv_ppo" or params['mode'] == 'adv_trpo' or params['mode'] == 'adv_sa_ppo' or params['mode'] == 'adv_pa_ppo' or params['mode'] == 'adv_acoe_ppo':
    ...
    if params['mode'] == "robust_q_ppo" or params['mode'] == 'adv_acoe_ppo':
        worst_q_table_dict = {
...
def add_common_parser_opts(parser):
    ...
    parser.add_argument('--mode', type=str, choices=['ppo', 'trpo', 'robust_ppo', 'adv_ppo', 'adv_trpo', 'adv_sa_ppo', 'adv_pa_ppo', 'robust_q_ppo', 'adv_acoe_ppo'],
                        help='pg alg')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp-id', type=str, default=None, required=False, help='load previous experiment and optimizer states before training')
    ...
    parser.add_argument('--norm-mode', type=str, default='A3B')
    parser.add_argument('--ccer', action='store_true', required=False)    
    parser.add_argument('--soft-ccer', action='store_true', required=False)
    parser.add_argument('--ccer_lambda', action='store_true', required=False)
    parser.add_argument('--soft_q_u_regret', action='store_true', required=False) 
    parser.add_argument('--ccer_wppo', action='store_true', required=False)    
    parser.add_argument('--soarl', action='store_true', required=False)
    parser.add_argument('--acoe_advs', action='store_true', required=False)
    parser.add_argument('--acoe_grad', action='store_true', required=False)
    ...
    params = vars(args)
    seed = params['seed']
    json_params = json.load(open('configs/'+args.config_path))

    extra_params = ['config_path', 'out_dir_prefix', 'load_model', 'no_load_adv_policy', 
                    'adv_policy_only', 'deterministic', 'seed', 'norm_mode', 'ccer', 'soft_ccer', 
                    'ccer_wppo', 'soarl', 'soft_q_u_regret', 'ccer_lambda', 'exp_id', 'acoe_advs', 'cuda_id', 'acoe_grad']
    ...
    if params['ccer_lambda'] or params['acoe_advs']:
        assert params['soft_ccer']
    if seed != -1:
