...
def main(params):
    ...
    excluded_params = ['config_path', 'out_dir_prefix', 'num_episodes', 'row_id', 'exp_id',
            'load_model', 'seed', 'deterministic', 'noise_factor', 'compute_kl_cert', 'use_full_backward', 'sqlite_path', 'early_terminate', 'cuda_id',  'compute_worst_q', 'load_q_model']
    ...
    if params['config_path']:
        ...
        json_params = json.load(open('configs/'+params['config_path']))
    ...
    if 'load_model' in params and params['load_model']:
        ...
        if params['row_id'] < 0:
            ...
            row_id = checkpoints['5_rewards'].idxmax()#params['row_id']
        ...
        if params['cpu'] == None:
            ...
            cpu = params['cpu']
        p, _ = Trainer.agent_from_data(store, row, cpu, extra_params=params, override_params=override_params, excluded_params=excluded_params, save_pt=False)
        p.params.ATTACK_ADVPOLICY_NETWORK = params['attack_advpolicy_network']
        for e in p.envs:
            e.normalizer_read_only = True
            e.setup_visualization(params['show_env'], params['save_frames'], params['save_frames_path'])
        store.close()
...
def get_parser():
    ...
    parser.add_argument('--soarl', action='store_true', help='Generates SOARL prior (not needed for testing)')
