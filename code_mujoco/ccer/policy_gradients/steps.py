...
def ccer_q_step(all_states, actions, next_states, not_dones, rewards, ccer_q_net, target_q_net, policy_net, min_q_net, ccer_opt,     
    ...
    for i in r:
        ...
        for selected in splits:
            ...
            worst_acts = worst_action_pgd(ccer_q_net, policy_net, sel_states, eps=0.01, maxiter=50)
            worst_next_actions = worst_action_pgd(ccer_q_net, policy_net, sel_next_states, eps=0.01, maxiter=50)
            next_acts = policy_net(ch.Tensor(sel_next_states))[0]
            
            #min_rews = target_q_net(ch.cat((sel_states, worst_actions), dim=1)).squeeze(-1) - target_q_net(ch.cat((sel_next_states, worst_next_actions),dim=1)).squeeze(-1)
            """Min reward = CCER - CCER_next ??? this is the method that worked really well for AAMAS, but the resulting equation is :
            # CCER(s,a) = r(s,a) - (CCER(s, a-)  - CCER(s', a-)) + CCER(s', a*), where a- is the worst-action
            which doesn't really make any intuitive sense, is self-referential, and also not substructure optimal, but it worked?
            """
            ...
            #expected_q = sel_rews + params.GAMMA * sel_not_dones * target_q_net(ch.cat((sel_next_states, worst_next_actions), dim=1)).squeeze(-1) #original wocar
            '''
            print('curr_q', curr_q.mean())
            print('expected_q', expected_q.mean())
            '''
    ...
    for _ in range(params.PPO_EPOCHS):
        if params.HISTORY_LENGTH > 0:
            ...
            for i, batch in enumerate(batches):
                ...
                mask = time_masks[i]
                batch_robust_q = target_q_net(ch.cat((batch_states, batch_actions), dim=1)).squeeze(-1)
                if optional_q_net:
                    batch_min_q = target_q_net(ch.cat((batch_states, batch_actions), dim=1)).squeeze(-1)
                '''
                print('batch states', batch_states.size())
                print('batch actions', batch_actions.size())
                print('batch old_log_ps', batch_old_log_ps.size())
                ...
                # dimension is the same as minibatch size.
                # We already normalized advs before. No need to normalize here.
                is_ccer = (params.ccer or params.soft_ccer or params.ccer_wppo or params.soarl)
            ...
            for selected in splits:
                ...
                # the surrogate rewards is basically exp(new_log_ps - old_log_ps) * advantage
                # dimension is the same as minibatch size.
                is_ccer = (params.ccer or params.soft_ccer or params.ccer_wppo or params.soarl or params.soft_q_u_regret)
