...
from .steps import value_step, step_with_mode, pack_history, worst_q_step, soft_ccer_step, ccer_q_step, soarl_q_step, soft_regret_q_u_step
from .logging import *
...
class Trainer():
    ...
    def __init__(self, policy_net_class, value_net_class, params,
        ...
        if self.MODE == 'robust_ppo' or self.MODE == 'adv_sa_ppo' or self.MODE == 'adv_pa_ppo' or self.MODE == 'robust_q_ppo' or self.MODE == 'adv_acoe_ppo':
            self.create_relaxed_model(time_in_state)

        # Minimax training
        if self.MODE == 'adv_ppo' or self.MODE == 'adv_trpo' or self.MODE == 'adv_sa_ppo' or self.MODE == 'adv_pa_ppo' or self.MODE == 'adv_acoe_ppo':
            ...
            if self.MODE == 'adv_pa_ppo' or self.MODE == 'adv_acoe_ppo':
        ...
        # Risk Q function optimization
        if self.MODE == 'robust_q_ppo' or self.MODE == 'radial_ppor' or self.MODE == 'adv_acoe_ppo':
            input_size = self.NUM_FEATURES if self.policy_model.discrete else self.NUM_FEATURES + self.NUM_ACTIONS
            self.q_model = value_net_class(input_size, self.INITIALIZATION, activation=self.value_activation)
            self.q_opt = optim.Adam(self.q_model.parameters(), lr=self.Q_LR, eps=1e-5) 
            self.target_q_model = deepcopy(self.q_model)
            
            self.min_q_model = value_net_class(input_size, self.INITIALIZATION, activation=self.value_activation)
            self.min_q_opt = optim.Adam(self.min_q_model.parameters(), lr=self.Q_LR, eps=1e-5) 
            self.target_min_q_model = deepcopy(self.min_q_model)
    ...
    def setup_stores(self, store):
        ...
        if self.params.MODE == 'robust_q_ppo' or self.MODE == 'adv_acoe_ppo':
            robust_q_cols ={
                'eps': float,
                'beta': float,
                'kl': float,
                ...
                'loss': float,
                'robust_q': float,
                'robust_q_weight': float,
                'current_value': float,
            }
            self.store.add_table('robust_q_ppo_data', robust_q_cols)
    ...
    def acoe_advantage_and_return(self, rewards, acoe, c_acoe, not_dones):
        """
        Calculate GAE advantage, discounted returns, and 
        true reward (average reward per trajectory) FOR ACOE
        ...
        V(s_t+1) = {0 if s_t is terminal
                   ...
                   {v_s if s_t not terminal and t == T
        """
        assert shape_equal_cmp(rewards, acoe, c_acoe, not_dones)
        
        V_s_tp1 = ch.cat([c_acoe[:,1:], c_acoe[:, -1:]], 1) * not_dones
        deltas = acoe + self.GAMMA * V_s_tp1 - c_acoe

        # now we need to discount each path by gamma * lam
        acoe_advantages = ch.zeros_like(rewards)
        returns = ch.zeros_like(rewards)
        indices = get_path_indices(not_dones)
        for agent, start, end in indices:
            acoe_advantages[agent, start:end] = discount_path( \
                    deltas[agent, start:end], self.LAMBDA*self.GAMMA)
            returns[agent, start:end] = discount_path( \
                    rewards[agent, start:end], self.GAMMA)

        return acoe_advantages.clone().detach(), returns.clone().detach()
    ...
    def run_trajectories(self, num_saps, return_rewards=False, should_tqdm=False,
        ...
        if collect_adversary_trajectory:
            ...
            assert self.MODE == "adv_ppo" or self.MODE == "adv_trpo" or self.MODE == "adv_sa_ppo" or self.MODE == "adv_pa_ppo" or self.MODE == 'adv_acoe_ppo'
            if self.MODE == "adv_pa_ppo" or self.MODE == 'adv_acoe_ppo':
        ...
        is_advpolicy_training = self.MODE == "adv_ppo" or self.MODE == "adv_trpo" or self.MODE == "adv_sa_ppo" or self.MODE == "adv_pa_ppo" or self.MODE == 'adv_acoe_ppo'
    ...
    def perturb_obs_fgsm(self, directions, last_states):
        ...
        with torch.enable_grad():
            ...
            if self.params.MODE == 'adv_acoe_ppo' and self.params.ACOE_GRAD:
                diff = self.q_model(ch.concat((last_states, old_action),dim=1)) - self.q_model(ch.concat((last_states, new_action),dim=1))
            else:
    ...
    def apply_attack(self, last_states):
        ...
        if self.params.ATTACK_METHOD == "critic":
            ...
            # Attack using a learned policy network.
            assert self.params.ATTACK_ADVPOLICY_NETWORK is not None
            if self.params.ATTACK_ADVPOLICY_NETWORK == 'exp':
    ...
    def collect_saps(self, num_saps, should_log=True, return_rewards=False,
        ...
        with torch.no_grad():
            ...
            if not test:
                ...
                if self.SOFT_CCER and self.ACOE_ADVS:
                    concat_states = ch.cat((trajs.states, trajs.actions), dim=-1)
                    next_actions = self.policy_model(trajs.next_states)[0]
                    concat_next_states = ch.cat((trajs.next_states, next_actions), dim=-1)
                    c_acoe = self.q_model.get_value(concat_states).squeeze(-1)
                    acoe = c_acoe - self.q_model(concat_next_states).squeeze(-1)
                    acoe_advantages, _ = self.acoe_advantage_and_return(trajs.rewards,
                                                acoe, c_acoe, trajs.not_dones)
                    if collect_adversary_trajectory:
                        acoe_advantages = -acoe_advantages
                    q_weight = (self.params.Q_WEIGHT/self.params.TRAIN_STEPS)  * float(self.n_steps)
                    advantages = advantages - q_weight*acoe_advantages
    ...
    def take_steps(self, saps, logging=True, value_only=False, adversary_step=False, increment_scheduler=True):
        ...
        if self.MODE == 'robust_q_ppo' or self.MODE == 'radial_ppor' or self.MODE == 'adv_acoe_ppo': 
            ...
            min_q_model = self.min_q_model
            target_min_q_model = self.target_min_q_model
            min_q_opt = self.min_q_opt
        ...
        if (self.MODE == 'robust_q_ppo' or self.MODE == 'radial_ppor' or self.MODE == 'adv_acoe_ppo') and not adversary_step:
            ...
            if self.CCER:
                ...
                q_loss = worst_q_step(saps.states, saps.actions, saps.next_states, saps.not_dones, saps.rewards, q_model, target_q_model, policy_model, q_opt, self.params, self.robust_eps_scheduler, should_cuda=not self.params.cpu)
            if self.ANNEAL_LR and increment_scheduler:
                q_scheduler.step()
            if logging:
                self.store.log_table_and_tb('optimization'+table_name_suffix, {
        ...
        if (self.MODE == 'robust_q_ppo' or self.MODE == 'robust_ppo' or self.MODE == 'adv_sa_ppo' or self.MODE == 'adv_pa_ppo' or self.MODE == 'adv_acoe_ppo') and not adversary_step and logging:
        ...
        if (self.MODE == 'robust_ppo' or self.MODE == 'adv_sa_ppo' or self.MODE == 'robust_q_ppo' or self.MODE == 'radial_ppor' or self.MODE == 'adv_acoe_ppo') and (isinstance(self.policy_model, CtsPolicy) or isinstance(self.policy_model, DiscCnnPolicy) or isinstance(self.policy_model, DiscPolicy)) and not adversary_step:
    ...
    def train_step_impl(self, adversary_step=False, increment_scheduler=True):
        ...
        if self.params.MODE == 'robust_q_ppo':
            ...
            policy_loss, surr_loss, entropy_bonus, val_loss = self.take_steps(saps, adversary_step=adversary_step, increment_scheduler=increment_scheduler)        # Logging code
    ...
    def run_test(self, max_len=2048, compute_bounds=False, use_full_backward=False, original_stdev=None, compute_worst_q=False):
        ...
        with torch.no_grad():
            ...
            if compute_worst_q:
                ...
                value = float("nan")
            # Unroll the trajectories (actors, T, ...) -> (actors*T, ...)
        return ep_length, ep_reward, actions.cpu().numpy(), action_means.cpu().numpy(), states.cpu().numpy(), kl_upper_bound, worst_q, value
    ...
    @staticmethod
    def agent_from_data(store, row, cpu, extra_params=None, override_params=None, excluded_params=None, save_pt=False):
        '''
        ...
        if extra_params['mode'] == 'adv_acoe_ppo':
            items.append('adversary_policy_model')
        ...
        if agent_params['mode'] == 'adv_acoe_ppo':
            load_state_dict(agent.adversary_policy_model, names['adversary_policy_model'])
        ...
        if agent.save_frames:
            for env in agent.envs:
                env.save_frames = agent.save_frames
        if save_pt:
            path = 'trained_models/'+ extra_params['game'] + extra_params['mode'] + '.pt'
            torch.save(agent.policy_model.state_dict(), path)
