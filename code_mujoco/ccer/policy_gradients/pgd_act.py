...
'''
def worst_action_pgd(q_net, policy_net, states, eps=0.0005, maxiter=100, return_std=False):
    ...
    for i in range(maxiter):
        ...
        if discrete:
            ...
            var_actions, std = policy_net(concat_states)
        else:
...
def worst_state_pgd(q_net, policy_net, states, eps=0.0005, maxiter=100):
    discrete = policy_net.discrete
    with torch.no_grad():
        action_ub, action_lb = network_bounds(policy_net, states, eps)
    ...
    for i in range(maxiter):
        concat_states = states if discrete else torch.cat((states, var_actions), dim=1)
        worst_q = q_net(concat_states)
        worst_q.backward(torch.ones_like(worst_q))
        grad = concat_states.grad.data if discrete else var_actions.grad.data 
        var_actions.data -= step_eps * torch.sign(grad)
